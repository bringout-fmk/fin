
all: 
	rm -f main/1g/e.obj
	make -C main/2g
	make -C db/1g
	make -C db/2g
	make -C dok/1g
	make -C razdb/1g
	make -C razoff/1g
	make -C rpt/1g
	make -C rptm/1g
	make -C sif/1g
	make -C specif/jerry/1g
	make -C specif/ramaglas/rpt/1g
	make -C sql/1g
	make -C sql/2g
	make -C budzet/1g
	#make -C dnal/1g
	make -C gendok/1g
	#make -C komp/1g
	make -C ostav/1g
	make -C param/1g
	make -C ut/1g
	make -C 1g
	

clean:
	cd 1g; make clean
	cd main/2g; make clean
	cd db/1g; make clean
	cd db/2g; make clean
	cd dok/1g; make clean
	cd razdb/1g; make clean
	cd razoff/1g; make clean
	cd rpt/1g; make clean
	cd rptm/1g; make clean
	cd sif/1g; make clean
	cd specif/jerry/1g; make clean
	cd specif/ramaglas/rpt/1g; make clean
	cd sql/1g; make clean
	cd sql/2g; make clean
	cd budzet/1g; make clean
	cd gendok/1g; make clean
	cd ostav/1g; make clean
	cd param/1g; make clean
	cd ut/1g; make clean
	make -C 1g  clean

zip:
	cd 1g; make zip

commit:
	cd 1g; make commit

